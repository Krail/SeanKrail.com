...
function verifySearchField(){var a=document.getElementById("search").value.trim(),b=!1,c=!1;console.log('Search: "'+a+'"');!a||0===a.length||/^[a-zA-Z0-9 ,.'\-]+$/.test(a)?($("#letters").slideUp({duration:500,easing:"easeInOutQuart"}),b=!0):$("#letters").slideDown({duration:500,easing:"easeInOutQuart"});60>=a.length?($("#length").slideUp({duration:500,easing:"easeInOutQuart"}),c=!0):$("#length").slideDown({duration:500,easing:"easeInOutQuart"});b&&c?document.getElementById("submitButton").removeAttribute("disabled"):
document.getElementById("submitButton").setAttribute("disabled",!0);console.log("Verifed Search Field: Valid characters? "+b+". Valid length? "+(c&&0!==a.length)+". Therefore valid search? "+(b&&c&&0!==a.length));return b&&c&&0!==a.length?!0:!1}
(function(){function a(){console.log("data sent");var a=new XMLHttpRequest,d=new FormData(b);a.addEventListener("load",function(a){if(200!==a.target.status)throw'Error: Server returned "'+a.target.status+" "+a.target.statusText+'"';a=JSON.parse(a.target.responseText);console.log(a);for(var b=0;b<a.projects.length;b++)document.getElementById("projects").insertBefore(document.getElementById(a.projects[b].id+"Article"),b+1!==a.projects.length?document.getElementById("projects").children[b]:null);for(b=
0;b<document.getElementsByTagName("mark").length;b++){for(var c=!1,d=0;d<a.keywords.length;d++){var f=document.getElementsByTagName("mark")[b].innerHTML.toLowerCase(),g=a.keywords[d].toLowerCase();if(f.includes(g)||g.includes(f))c=!0}c?console.log("Match"):console.log("No Match");c?document.getElementsByTagName("mark")[b].setAttribute("class","searched"):document.getElementsByTagName("mark")[b].removeAttribute("class")}});a.addEventListener("error",function(a){alert("Oops! Something goes wrong.")});
a.open("POST","/projects");a.send(d)}var b=document.getElementById("searchForm");b.addEventListener("submit",function(b){verifySearchField()?!1===JSON.parse(document.getElementById("reload").value)&&(console.log("form submitted valid search"),b.preventDefault(),a()):(console.log("form not submitted: invalid search"),b.preventDefault())})})();
