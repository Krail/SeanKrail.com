...
var auto_peer = new AutoPeer();
auto_peer.on('autoPeer:connected', (_id) => {
	id = _id;
	//auto_peer.broadcast()
});
auto_peer.on('followme-broadcast', (_message, _data) => {
	if (_message && _message.userid && _message.geoposition && _message.geoposition.timestamp && _message.geoposition.latitude && _message.geoposition.longitude) {
		if (worker && worker.port) {
			var pos = {
				lat: _message.geoposition.latitude,
				lng: _message.geoposition.longitude
			};

			markers.push(new UserMarker(_message.userid, _message.geoposition.latitude, _message.geoposition.longitude, _message.geoposition.accuracy, _message.geoposition.heading));

			if (markers.length > 1) {
				var my_markers = [];
				markers.forEach((_marker, _index, _array) => {
					if (_marker.user.id === _message.userid) {
						my_markers.push(_marker);
						...
						if (_marker.marker.heading && _marker.marker.heading.getAnimation()) _marker.marker.heading.setAnimation(null);
					}
				});
				if (my_markers.length > 1) {
					var prev_marker = my_markers[my_markers.length-2];
					paths.push(new UserPath(_message.userid, prev_marker.user.latitude, prev_marker.user.longitude, geoposition.latitude, geoposition.longitude));
				}
			}
			...
			var message = new SetMessage(_message.userid, _message.geoposition.timestamp, _message.geoposition);
			worker.port.postMessage(message);

		} else console.log(TAG + ERROR + 'Worker or Worker.port is not instantiated.');
	} else console.log(TAG + ERROR + 'Received message in invalid format from a broadcasted message.');
});
...
function initMap() {
	...
	if ('geolocation' in window.navigator) {
		...
		geolocation.watchPosition((_position) => {
			...
			var message = new SetMessage(null, geoposition.timestamp, geoposition);
			...
			if (id) auto_peer.broadcast('followme-broadcast', { userid: id, geoposition: geoposition }});

			markers.push(new UserMarker(null, geoposition.latitude, geoposition.longitude, geoposition.accuracy, geoposition.heading));
			...
			if (markers.length > 1) {
				...
				paths.push(new UserPath(null, prev_marker.user.latitude, prev_marker.user.longitude, geoposition.latitude, geoposition.longitude));
			}
	...
	*/
	function UserMarker(_userid, _latitude, _longitude, _accuracy, _heading) {
		this.user = {
			id: _userid,
			latitude: _latitude,
		...
		point = MyPoint(_userid, _latitude, _longitude, opacity);

		if (_accuracy && _accuracy > 5) accuracy = MyAccuracy(_userid, _latitude, _longitude, _accuracy, opacity);
		else accuracy = null;

		if (_heading) heading = MyHeading(_userid, _latitude, _longitude, _heading, opacity);
		else heading = null;
	...
	function UserPath(_userid, _start_latitude, _start_longitude, _end_latitude, _end_longitude) {
		this.user = {
			id: _userid,
...
*/
function MyPoint(_userid, _latitude, _longitude, _opacity) {
	...
	return new google.maps.Marker({
		...
		icon: {
			anchor: {x: 0, y: 0},
			fillColor: (_userid === null || _userid === id) ? '#00FF00' : '#FF00FF',
			fillOpacity: 0.9,
			labelOrigin: {x: 0, y: 0},
			path: google.maps.SymbolPath.CIRCLE,
			rotation: 0,
			scale: 8,
			strokeColor: (_userid === null || _userid === id) ? '#66FF66' : '#FF66FF',
			strokeOpacity: 0.9,
			strokeWeight: 3
		},
		label: '',
...
*/
function MyAccuracy(_userid, _latitude, _longitude, _accuracy, _opacity) {
	return new google.maps.Circle({
		center: {lat: _latitude, lng: _longitude},
		clickable: false,
		draggable: false,
		editable: false,
		fillColor: (_userid === null || _userid === id) ? '#FF0000' : '#FFFF00',
		fillOpacity: _opacity,
		map: map,
		radius: _accuracy,
		strokeColor: (_userid === null || _userid === id) ? '#FF6666' : '#FFFF66',
		strokeOpacity: 2 * _opacity,
		strokePosition: google.maps.StrokePosition.CENTER,
		strokeWeight: 5,
		visible: true,
...
*/
function MyHeading(_userid, _latitude, _longitude, _heading, _opacity) {
	return new google.maps.Marker({
		anchorPoint: {x: 0, y: 0},
		animation: google.maps.Animation.DROP,
		attribution : {},
		...
		icon: {
			anchor: {x: 0, y: 0},
			fillColor: (_userid === null || _userid === id) ? '#0000FF' : '#00FFFF',
			fillOpacity: _opacity,
			labelOrigin: {x: 0, y: 0},
			path: google.maps.SymbolPath.FORWARD_CLOSED_ARROW,
			rotation: _heading,
			scale: 5,
			strokeColor: (_userid === null || _userid === id) ? '#0000FF' : '#00FFFF',
			strokeOpacity: 2 * _opacity,
			strokeWeight: 5
		},
		label: '',
